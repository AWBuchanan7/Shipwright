...
// Drink effects are 1 -> strong heal, 2 -> fill magic, 4 -> weak heal.
// Effects can be combined through addition.
static u8 sNewBottleDrinkEffects[] = {
...
u16 NewItem_ActionParamToBoots(s16 actionParam) { // [TO-DO]
    return actionParam - PLAYER_IA_BOOTS_KOKIRI + 1;
}

u16 NewItem_ActionParamToTunic(s16 actionParam) { // [TO-DO]
...
void *NewItem_GetBlureArrow(s16 arrowType) { // [TO-DO]: Add to cosmetics
...
s32 NewItem_GetCutsceneItem(s16 actionParam) { // 15 vanilla cutscene items [TO-DO]: Figure out bean.
...
s32 NewItem_GetExchangeItem(s32 tradeItem, s32 bottle) { 
    // [TO-DO]: Figure out way to allow for easy adding of new exchange items 
...
s16 NewItem_ItemToActionParam(s32 newItem) {
    return sNewItemActionParams[newItem - ITEM_NUT_UPGRADE_40 - 1];
}
